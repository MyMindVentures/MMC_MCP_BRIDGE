...
# Of restart (code changes via volume mount)
...
```bash
# Rebuild (altijd nodig voor production)
...
```bash
# Rebuild na dependency of tool changes
...
- [ ] Container start zonder errors
- [ ] Health endpoint reageert: `curl http://localhost:3001/api/health`
- [ ] SSE endpoint werkt: `curl http://localhost:3001/api/sse`
- [ ] i18n werkt (check language switcher)
- [ ] Public assets laden (check manifest.json, sw.js)
