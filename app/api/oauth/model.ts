...
// Save token
export async function saveToken(token: any, client: any, user: any): Promise<OAuth2Token> {
  const pool = getPgPool();
...
export async function revokeToken(token: OAuth2Token): Promise<boolean> {
  ...
  await pool.query(
    'DELETE FROM oauth2_tokens WHERE refresh_token = $1',
    [token.refreshToken]
  );
...
export async function saveAuthorizationCode(code: any, client: any, user: any): Promise<OAuth2AuthorizationCode> {
  const pool = getPgPool();
...
export async function revokeAuthorizationCode(code: OAuth2AuthorizationCode): Promise<boolean> {
  const pool = getPgPool();
...
// Verify scope
export async function verifyScope(token: OAuth2Token, scope: string): Promise<boolean> {
...
export async function getUser(username: string, password: string): Promise<OAuth2User | null> {
...
export async function getUserFromClient(client: OAuth2Client): Promise<OAuth2User> {
  return {
    id: client.clientId,
...
export function validateRedirectUri(redirectUri: string, client: OAuth2Client): boolean {
