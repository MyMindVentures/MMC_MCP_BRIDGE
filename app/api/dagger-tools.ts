...
let daggerClient: Client | null = null;
...
export async function runPipeline(pipelineName: string, params?: any): Promise<any> {
...
export async function buildImage(context: string, dockerfile?: string, tag?: string): Promise<any> {
  return withClient(async (client) => {
...
export async function buildWithCache(context: string, cacheKey: string): Promise<any> {
  return withClient(async (client) => {
...
export async function deployToRailway(image: string, serviceName: string): Promise<any> {
  return withClient(async (client) => {
    ...
    const result = await client
      ...
      .withExec(['railway', 'deploy', '--service', serviceName, '--image', image])
      .stdout();
...
export async function deployToDocker(image: string, containerName: string): Promise<any> {
...
export async function runTests(context: string, testCommand?: string): Promise<any> {
  return withClient(async (client) => {
    const result = await client
      .container()
      .from('node:20')
...
export async function createModule(name: string, definition: any): Promise<any> {
...
// MAIN EXECUTOR
export async function executeDaggerTool(tool: string, params: any): Promise<any> {
  try {
    switch (tool) {
      // Pipelines
      case 'createPipeline':
