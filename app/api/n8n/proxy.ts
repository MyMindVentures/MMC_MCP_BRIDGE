...
async function initializeClient() {
  ...
  try {
    ...
    const baseUrl =
      process.env.N8N_BASE_URL || "https://mmc-n8n-instance.up.railway.app";

    if (!apiKey) {
      console.log(
        "[n8n-community] N8N_INSTANCE_APIKEY or N8N_API_KEY not configured, skipping",
      );
      return null;
    }
...
export async function executeN8NCommunityTool(toolName: string, params: any) {
  try {
    ...
    console.error(
      `[n8n-community] Failed to execute ${toolName}:`,
      error.message,
    );
    throw error;
  }
